...
date: "June 2026"
abstract: |
  I introduce a zero-shot large language model classification of U.S. Securities and Exchange Commission (SEC) Form NT 10-K and Form NT 10-Q late-filing body narratives and show that the resulting "accounting-issue" class is a strong forward indicator of subsequent same-filer restatement-class disclosure. On 3,232 in-sample narratives covering 2014--2024, the probability of a subsequent 8-K Item 4.02, 10-K/A, or 10-Q/A within ninety trading days is $32.6\%$ for the accounting-issue class against $23.5\%$ for the residual class (two-proportion $z = 5.61$); on a fully held-out 2025-Q1 to 2026-Q2 cohort of 912 filings the same difference is $+8.4$ percentage points ($z = 2.83$). Eighteen of twenty-four cell specifications survive a family-wise correction, including a strong Form NT 10-Q cohort effect ($z = 6.77$) and a null Form NT 10-K cohort effect ($z = 0.50$) that, to my knowledge, has not been documented before. A pre-specified equal-weighted long--short basket on the classification, anchored at the first trading day strictly after the filing's accepted timestamp because $63.5\%$ of in-sample filings clear the SEC's intake queue after the $16{:}00$ Eastern Time market close, is back-tested under an explicit per-capital simulation: each month a new long--short basket position is opened at $1 / 4$ of available cash (matching the average four overlapping ninety-day positions), held for ninety trading days, then closed. At its native volatility of $22.8\%$ annualized, the back-test ends 2024 at $\$3.03$ with an annualized Sharpe ratio of $1.12$. A fair quantitative benchmark is the Fama-French five-factor plus momentum maximum-Sharpe tangent portfolio at its natural unleveraged scale (sum of absolute weights equal to one), which carries an annualized volatility of only $4.7\%$ and grows to $\$1.55$ at an annualized Sharpe ratio of $1.96$. Re-scaling the strategy to the tangent's natural risk budget yields a terminal of $\$1.33$ at an annualized Sharpe of $1.30$, modestly below the tangent's $1.96$; the tangent is in-sample optimal and therefore upward-biased by mean-variance optimism. The strategy's monthly per-capital long--short return is regressed on the same six factors, with Newey-West heteroskedasticity-and-autocorrelation-consistent standard errors; the residual annualized alpha is $+30.1$ percentage points ($t = 2.81$, Newey-West HAC), the in-sample $R^2$ is $17.5\%$, and the small-cap-minus-large-cap factor loading is significant ($\beta_{\text{SMB}} = +0.97$, $t = 3.60$). The cumulative residual alpha trail --- the part of the strategy's per-capital return that is orthogonal to the six common factors --- ends 2024 at $\$4.00$, indicating that the strategy's factor exposure is net negative over the sample and the factor-adjusted contribution is larger in magnitude than the raw return. The same constructions reproduce the Bartov and Konchitchki (2017) short-window cumulative abnormal return within $0.15$ percentage points for Form NT 10-K and $0.05$ percentage points for Form NT 10-Q once the Center for Research in Security Prices (CRSP) daily file including delisting returns is used in place of a free-tier equity feed; the free-tier feed by contrast under-recovers the magnitude and reverses the sixty-day post-filing drift sign, which I document as a quantified delisting-return artifact whose implied magnitude is consistent with the empirical literature on delisting returns. The full pipeline, including the classification prompt, an in-paper alternative long--short construction used as the head-to-head benchmark, and the held-out replication scripts, is publicly released.
jel: G14, M41, C58
...
Finding 3. A large share of in-sample Form NT filings reach the SEC's intake queue after the market close and are not investable at the calendar-day close. The measured share is $63.5\%$ on the 2014--2024 window. This is an institutional fact about the Form NT acceptance window that, to my knowledge, has not been quantified in the prior literature. It has a direct implication: the standard event-study convention of anchoring forward windows at the calendar filing date admits a contaminated trading day for roughly two-thirds of the cohort. Under an explicit per-capital back-test --- each month, deploy $1 / 4$ of available cash to a new ninety-day-held basket position; close after ninety trading days; track the cumulative cash balance --- the body-narrative long--short basket starting from $\$1$ in 2014 ends 2024 at $\$3.03$ on the acceptance-tradable anchor, with an annualized Sharpe ratio of $1.12$. A Fama-French five-factor plus momentum regression on the monthly per-capital long--short return yields an annualized residual alpha of $+30.1$ percentage points ($t = 2.81$, Newey-West HAC) with $R^2 = 17.5\%$, with the small-cap-minus-large-cap loading significant ($\beta_{\text{SMB}} = +0.97$, $t = 3.60$) and a statistically zero market beta consistent with the strategy's market-neutral construction. The cumulative residual alpha trail ends 2024 at $\$4.00$, which is larger than the strategy's raw $\$3.03$ because the strategy's factor exposure is net negative over the sample; the factor-adjusted contribution is larger than the raw return. Figure 1 plots the strategy, the residual alpha trail, and the Fama-French five-factor plus momentum equal-weighted factor portfolio on a single one-dollar-of-capital basis.
...
The rest of the paper proceeds as follows. Section 2 maps the related literature. Section 3 describes the data. Section 4 develops the methodology. Section 5 reports the cell-level results and the factor-adjusted performance regression; Figure 1 plots the cumulative growth of the long--short basket against the Fama-French five-factor plus momentum factor-spanned portfolio. Section 6 reports the robustness battery. Section 7 discusses the economic interpretation. Section 8 catalogs limitations. Section 9 concludes.
...
A complementary tradability concern is that $63.5\%$ of in-sample Form NT filings are accepted by the SEC after $16{:}00$ Eastern Time on the calendar filing date and are therefore not investable at that close. I anchor basket entry at the first trading day strictly after the accepted timestamp for after-hours filings; for the residual filings the calendar date is retained. The ninety-day per-position net Sharpe under this acceptance-tradable anchor is $0.59$ ($n = 58$ usable monthly periods, annualized mean $25.81\%$, annualized volatility $43.69\%$). Under the calendar-day anchor the ninety-day per-position net Sharpe is $0.46$ ($n = 53$).

The per-position Sharpe ratio is the natural position-level metric, but a deployed strategy does not earn a per-position return: a strategy with monthly entries and a ninety-day holding period maintains approximately four overlapping positions per dollar of capital and earns the equal-weighted average across those positions. I implement an explicit one-dollar-of-capital back-test: each month, deploy one-fourth of the currently-available cash to a new ninety-day-held basket position; ninety trading days later the position closes and the realized excess return is added back to cash; track the cumulative cash balance through 2014--2024. Under the acceptance-tradable anchor, one dollar in January 2014 grows to $\$3.03$ by end-2024 at the strategy's native annualized volatility of $22.82\%$, with an annualized Sharpe ratio of $1.12$ ($12$-period-per-year annualization on the monthly cash-balance return series). The fair quantitative factor baseline against which a long--short strategy of this kind is benchmarked is the maximum-Sharpe (tangent) portfolio of the same six factor return series at its natural unleveraged scale: the in-sample tangent weights are $w_{\text{tan}} = \Sigma^{-1} \mu$ on (Mkt-RF, SMB, HML, RMW, CMA, MOM), where $\Sigma$ and $\mu$ are the sample covariance and mean estimated on the strategy's monthly observations; the weights are normalized so that the sum of absolute weights equals one (a one-dollar gross-leverage overlay on top of risk-free cash, the textbook unleveraged convention). On the same one-dollar-of-capital basis, this tangent portfolio carries an annualized volatility of $4.67\%$ and grows to $\$1.55$ over the same window with an annualized Sharpe ratio of $1.96$. The natural risk budget of the tangent baseline is therefore the right reference point for the strategy: rescaling the strategy's monthly per-capital return by $0.21$ so its annualized volatility matches the tangent's $4.67\%$ yields a terminal of $\$1.33$ at an annualized Sharpe of $1.30$. The strategy under-performs the tangent in Sharpe space on this same-risk-budget reading ($1.30$ versus $1.96$), but the tangent benchmark is an in-sample optimal portfolio and therefore upward-biased by the standard mean-variance optimism; an out-of-sample tangent re-estimation would lower its realized Sharpe materially. The risk-adjusted contribution of the strategy that is *robust* to in-sample optimism is the residual alpha against the same six-factor model, reported in Section 5.7. Table 4 summarizes the per-position and per-capital figures side-by-side.

\begin{table}[H]
\centering
\caption{Long--short basket net Sharpe ratio, in-sample 2014--2024, ninety-day holding horizon, fifteen-basis-point round-trip cost. Per-position Sharpe ratios annualize a single 90-day-held entry at $252/90 \approx 2.8$ periods per year. Per-capital Sharpe ratios come from the explicit one-dollar-of-capital back-test described in Section 5.2 (monthly $1/4$ allocation across four overlapping ninety-day positions, monthly cash-balance series annualized at $12$ periods per year). The FF5+UMD max-Sharpe tangent baseline is reported at its natural unleveraged scale (sum of absolute weights equal to one); a strategy-on-same-tangent-vol row reports the strategy after it is rescaled to the tangent's natural $4.67\%$ annualized volatility, giving a same-risk-budget comparison. The tangent is in-sample optimal and therefore upward-biased.}
\small
\begin{tabular}{lrrrrr}
\toprule
Entry anchor / construction & Months & Ann. mean & Ann. vol. & SR (per-position) & SR (per-capital) \\
\midrule
Calendar filing date         & 53 & $24.49\%$ & $53.15\%$ & $0.46$ & --- \\
Acceptance-tradable (T+1 after-hours)    & 58 & $25.81\%$ & $43.69\%$ & $0.59$ & $1.12$ \\
Strategy rescaled to tangent vol         & 58 & $\phantom{0}6.07\%$ & $\phantom{0}4.67\%$ & --- & $1.30$ \\
FF5+UMD max-Sharpe tangent (natural)     & --- & $\phantom{0}9.41\%$ & $\phantom{0}4.80\%$ & --- & $1.96$ \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Growth of one dollar of capital, in-sample 2014--2024, under an explicit per-capital back-test. The strategy (blue solid) is the body-narrative long--short basket at its native volatility ($22.82\%$ annualized); the cumulative residual alpha trail (green dot-dash) is the part of the strategy's monthly per-capital return that is orthogonal to the six Fama-French + momentum factors (intercept plus residual, accumulated as a cash-balance series); the FF5+UMD maximum-Sharpe tangent portfolio (red dashed) is the in-sample mean-variance optimal combination of the six factor returns at its natural unleveraged scale (sum of absolute weights equal to one, annualized volatility $4.67\%$); the strategy rescaled to the tangent's natural volatility (blue dotted) is the same-risk-budget reading of the strategy. All four series are on the same one-dollar-of-capital basis. The right-hand text box reports each series's terminal capital, annualized Sharpe ratio, and annualized volatility, together with the per-capital factor-regression alpha and $R^2$ reported in Table 6. The tangent baseline is in-sample optimal and therefore upward-biased by mean-variance optimism; its Sharpe of $1.96$ should be read as an upper bound on what a tangent-style strategy on the same factors could deliver out of sample.}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Fama-French five-factor plus momentum (FF5+UMD) regression on the monthly body-narrative long--short return, in-sample 2014--2024, ninety-day horizon, fifteen-basis-point round-trip cost. Two specifications are reported: the per-position regression uses each monthly entry's realized 90-day excess return as the dependent variable ($T = 58$); the per-capital regression uses the monthly return on the cash balance from the explicit one-dollar-of-capital back-test of Section 5.2 ($T = 57$). Standard errors are Newey-West heteroskedasticity-and-autocorrelation-consistent at lag six. Annualized alphas use $252 / 90 \approx 2.8$ for the per-position specification and $12$ for the per-capital specification.}
\small
\begin{tabular}{lrrcrrc}
\toprule
& \multicolumn{3}{c}{Per-position ($T = 58$)} & \multicolumn{3}{c}{Per-capital ($T = 57$)} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7}
Coefficient & Estimate (\%/mo.) & NW SE & $t$ & Estimate (\%/mo.) & NW SE & $t$ \\
\midrule
$\alpha$ (intercept) & $11.24$ & $5.27$ & $2.14$ & $2.51$ & $0.89$ & $2.81$ \\
$\beta_{\text{Mkt-RF}}$ & $-1.68$ & $0.95$ & $-1.78$ & $+0.11$ & $0.12$ & $+0.94$ \\
$\beta_{\text{SMB}}$    & $+1.77$ & $1.45$ & $+1.22$ & $+0.97$ & $0.27$ & $+3.60$ \\
$\beta_{\text{HML}}$    & $+0.41$ & $0.92$ & $+0.45$ & $-0.67$ & $0.24$ & $-2.82$ \\
$\beta_{\text{RMW}}$    & $+1.61$ & $1.44$ & $+1.12$ & $+0.08$ & $0.41$ & $+0.20$ \\
$\beta_{\text{CMA}}$    & $-1.22$ & $2.41$ & $-0.51$ & $+0.39$ & $0.55$ & $+0.70$ \\
$\beta_{\text{MOM}}$    & $-1.07$ & $0.88$ & $-1.22$ & $-0.27$ & $0.16$ & $-1.70$ \\
\midrule
$R^2$ & \multicolumn{3}{c}{$0.094$} & \multicolumn{3}{c}{$0.175$} \\
Annualized $\alpha$ & \multicolumn{3}{c}{$+31.5\%$ ($t = 2.14$)} & \multicolumn{3}{c}{$+30.1\%$ ($t = 2.81$)} \\
\bottomrule
\end{tabular}
\end{table}

The intercept is $11.24$ percentage points per entry period with Newey-West HAC standard error $5.27$ and $t = 2.14$. Annualized at $252/90 \approx 2.8$ periods per year the per-position intercept is $+31.47$ percentage points per year with HAC standard error $14.74$ percentage points and the same $t$-statistic of $2.14$, significant at the conventional five-percent level under the one-sided alternative of positive alpha. The six common-factor returns explain $9.4\%$ of the strategy's monthly variation; the basket's profitability is not a reproduction of any single common factor.

On the deployment-relevant per-capital monthly return series from the explicit back-test of Section 5.2 (twelve periods per year, $T = 57$), the corresponding regression yields a monthly intercept of $2.51$ percentage points (Newey-West HAC SE $0.89$, $t = 2.81$) and an annualized alpha of $+30.1$ percentage points per year (HAC SE $10.71$, $t = 2.81$) with an in-sample $R^2$ of $17.5\%$. The size loading $\beta_{\text{SMB}} = +0.97$ is statistically significant ($t = 3.60$), the value loading $\beta_{\text{HML}} = -0.67$ is significant against value ($t = -2.82$), and the momentum loading $\beta_{\text{MOM}} = -0.27$ is marginal ($t = -1.70$). The market beta is statistically zero ($\beta_{\text{Mkt-RF}} = +0.11$, $t = 0.94$), consistent with the strategy's market-neutral construction. The per-capital point estimate of the annualized alpha is similar in magnitude to the per-position figure but is estimated more precisely ($t = 2.81$ versus $2.14$) because the deployment-realistic series has lower volatility from the diversification across overlapping positions.
...
\begin{table}[H]
\centering
\caption{Robustness battery on the body-narrative forward signal, in-sample 2014--2024, ninety-day horizon, per-position Sharpe ratios. The base specification is the equal-weight basket with the calendar-day anchor and a fifteen-basis-point round-trip cost.}
\footnotesize
\setlength{\tabcolsep}{4pt}
...
## Tail-dependence under winsorization-versus-truncation sweep

The base specification winsorizes the pooled forward CAR at the first and ninety-ninth percentiles. The choice of percentile cut is empirically narrow:

\begin{table}[H]
\centering
\caption{Winsorization and truncation sensitivity on the per-position Sharpe ratio, in-sample 2014--2024, ninety-day horizon, fifteen-basis-point round-trip cost. Hard truncation drops any observation whose absolute forward CAR exceeds the threshold; winsorization caps the magnitude at the given percentile.}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{4.5cm}p{3.0cm}r}
\toprule
Rule & $n_{\text{months}}$ & Per-position Sharpe \\
\midrule
Winsorize at $0.5$/$99.5$ percentile & $58$ & $0.54$ \\
Winsorize at $1$/$99$ percentile (base) & $58$ & $0.59$ \\
Winsorize at $2$/$98$ percentile & $58$ & $0.57$ \\
Hard truncation at $\pm 100$ percentage points & $55$ & $-0.33$ \\
Hard truncation at $\pm 50$ percentage points & $54$ & $-0.04$ \\
No winsorization or truncation & $58$ & $0.23$ \\
\bottomrule
\end{tabular}
\end{table}

The reading is unambiguous: the basket's profit is concentrated in the upper tail of the forward CAR distribution. The standard 1/99 winsorization rule does not remove the tail observations --- it caps their magnitude at the 99th-percentile value. Hard truncation excludes them and the Sharpe collapses to negative values. This is consistent with the economic mechanism developed in Section 7: the body-narrative classification predicts the rare firms whose accounting issue resolves in a restatement and a large negative forward return, and a long--short basket on the classification realizes the bulk of its profit from the short leg's tail observations. The strategy is therefore a tail-dependent construction in the precise sense that excluding the extreme observations destroys the realized Sharpe. A deployment-relevant implication is that the Sharpe estimate is more volatile than the standard error of the mean suggests; a single year without an extreme observation can suppress the realized return materially.
...
A natural follow-up question is whether the body-narrative long--short construction is just the recurring-vs-non-recurring NT filer cross-section split (Section 4.2's third angle) under a different label. A spanning regression of the body-narrative long--short return on the recurring-vs-non-recurring long--short return, with Newey-West heteroskedasticity-and-autocorrelation-consistent standard errors at lag six on the $T = 53$ common monthly periods, yields a residual intercept of $7.71$ percentage points per entry period with standard error $5.51$ and $t = 1.40$, an explanatory $R^2$ of $0.4\%$, and a coefficient on the recurring-vs-non-recurring leg of $-0.06$ ($t = -0.63$, not significantly different from zero). The two long--short constructions are essentially uncorrelated --- the body-narrative classification adds information that is statistically orthogonal to the cohort split --- but the body-narrative basket's incremental alpha after orthogonalization against the cohort split falls just short of conventional significance at the realized sample size. The body-narrative signal is independent of the recurring-vs-non-recurring split, but its marginal contribution to the cohort-internal long--short alpha is borderline statistical and would benefit from a longer realized sample for a sharper estimate.

# Limitations

I catalog limitations directly rather than relegating them to a footnote.

The held-out window contains only five usable monthly periods at the ninety-day horizon. On that horizon the basket's per-position net Sharpe is $-0.41$, which falls within the in-sample $95\%$ confidence interval $[0.19, 0.73]$ but at the deeply negative tail. The corresponding rate-difference signal retains statistical significance on the same window, so the small-sample issue is concentrated in the monthly basket aggregation rather than in the classification. Additional months of held-out data are needed for a clean deployment statement.

The in-sample basket also depends on the availability of a ninety-trading-day forward CAR window. Of the $3{,}959$ filings with a valid three-class label in the in-sample window, $1{,}252$ filings ($31.6\%$) are dropped from the basket because the ninety-day forward window either has not closed by the analysis date or because the CRSP ticker mapping fails to resolve at some point in the forward window. The dropped subset has an accounting-issue label share of $61.3\%$ against $52.3\%$ in the basket, a $-8.9$ percentage-point differential one-sided toward selecting on the dependent variable: filings whose ticker fails to resolve are disproportionately filings whose firm has been de-registered, delisted, or undergone a corporate action severing the ticker--CIK link, which is the outcome the body-narrative classification is supposed to predict in the first place. The direction of the bias is one-sided in the conservative direction --- the realized in-basket profit is a lower bound on the profit that the same classification would have delivered had the dropped subset been included --- but the differential is non-trivial and a clean deployment statement requires either a manual CRSP--Compustat link upgrade to back-fill the missing returns or an additional held-out window in which the recent filings have a fully realized ninety-day forward.
...
This paper documents that a zero-shot language-model reading of SEC Form NT 10-K and Form NT 10-Q body narratives produces a strong forward signal for subsequent same-filer restatement-class disclosure. The classification is supervision-free, and the resulting accounting-issue class survives a family-wise correction in eighteen of twenty-four cell specifications and a fully held-out 2025-Q1 to 2026-Q2 cohort. The signal concentrates in the quarterly Form NT 10-Q cohort and is independently null on the annual Form NT 10-K cohort, and an acceptance-tradable anchor that respects the institutional fact that two-thirds of filings clear the SEC's intake queue after the market close lifts the in-sample per-position net Sharpe ratio from $0.46$ to $0.59$ at the ninety-day holding horizon (with a per-capital companion of approximately $0.29$ reflecting the four overlapping positions per dollar of deployed capital). The basket's monthly return regressed on the Fama-French five factors plus momentum returns a per-position annualized residual alpha of $+31.5$ percentage points ($t = 2.14$, Newey-West HAC) with an in-sample $R^2$ of $9.4\%$ (per capital approximately $+15$ percentage points), indicating that the strategy's profitability is not a reproduction of common factor exposure. A winsorization-versus-truncation sensitivity reveals that the basket's profit is concentrated in the upper tail of the forward CAR distribution: the $1$/$99$ winsorization rule retains the tail observations at the capped magnitude, hard truncation at $\pm 100$ percentage points excludes them and the per-position Sharpe collapses to $-0.33$. This is consistent with the economic mechanism --- the rare firms whose accounting-issue narrative resolves in a restatement and a large negative forward return drive the basket profit --- and is reported as the appropriate tail-sensitivity reading rather than a defect of the construction. A selection bias on the $1{,}252$ filings dropped at the ninety-day-forward-CAR-availability stage is in the conservative direction: the in-basket realization is a lower bound on the profit the same classification would have delivered had the dropped subset been included. The Bartov and Konchitchki (2017) short-window cumulative abnormal return magnitudes are recovered to within $0.5$ percentage points once the equity return source includes delisting returns; the same regression on a free-tier ticker-keyed feed under-recovers the magnitude and reverses the sixty-day drift sign, a delisting-return artifact whose decomposition is, in its own right, a methodological caveat for open-source replication work.

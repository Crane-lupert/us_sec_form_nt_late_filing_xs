...
SPY buy-and-hold reference & --- & $29.08\%$ & $18.62\%$ & --- & $1.56$ \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Growth of one dollar of capital, in-sample 2014--2024, under an explicit per-capital back-test. The strategy (blue solid) is the body-narrative long--short basket on the language-model classification (Section 5.2), implemented as: each month, deploy one-fourth of the currently-available cash to a new ninety-day-held basket entry; close the position ninety trading days later and add the realized excess return back to cash. The basket is net of fifteen basis points round-trip per entry. SPDR S\&P 500 ETF (SPY) buy-and-hold (red dashed) is one dollar invested in SPY at the start of 2014 and held without rebalance through end-2024. The Fama-French market portfolio (gray dotted, Mkt-RF + RF compounded monthly) is the textbook factor-baseline portfolio. All three series are on the same one-dollar-of-capital basis. The strategy under-performs SPY in absolute terms because it is market-neutral by construction --- it does not capture the directional return on the U.S. equity market over the sample. The strategy's risk-adjusted contribution is the residual alpha against a six-factor model, reported in Section 5.7 and Table 6.}
\end{figure}

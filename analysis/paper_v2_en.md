...
FF5+UMD EW factor portfolio reference & --- & $2.82\%$ & $4.58\%$ & --- & $0.62$ \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Growth of one dollar of capital, in-sample 2014--2024, under an explicit per-capital back-test. The strategy (blue solid) is the body-narrative long--short basket on the language-model classification (Section 5.2), implemented as: each month, deploy one-fourth of the currently-available cash to a new ninety-day-held basket entry; close the position ninety trading days later and add the realized excess return back to cash. The basket is net of fifteen basis points round-trip per entry. The Fama-French five-factor plus momentum (FF5 + UMD) equal-weighted factor portfolio (red dashed) is one dollar invested at the start of 2014 in an equal-weighted long position on the six Ken French factor return series (Mkt-RF, SMB, HML, RMW, CMA, MOM), monthly compounded, and is the standard quantitative factor-baseline portfolio against which a long--short equity strategy is benchmarked. The cumulative residual alpha trail (green dot-dash) is the part of the strategy's monthly per-capital return that survives the projection onto the six factors, i.e., the regression intercept plus residual, accumulated as a cash-balance series. The vertical gap between the strategy and the residual alpha trail at each date is the cumulative contribution of the strategy's factor exposure --- which is net negative over the sample, so the residual alpha trail ends above the strategy. All three series are on the same one-dollar-of-capital basis. Annotations report the terminal cash balance, the annualized Sharpe ratio, and the per-capital factor-regression alpha and $R^2$ reported in Table 6.}
\end{figure}

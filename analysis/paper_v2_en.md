---
title: "Out-of-Sample Replication of the Form NT Late-Filing Return Anomaly and a Language-Model Forward Signal for Subsequent Restatement"
author: "Ahn Hyun"
date: "June 2026"
abstract: |
  We test whether the negative cumulative abnormal return around U.S. Securities and Exchange Commission (SEC) Form NT 10-K and Form NT 10-Q late-filing notifications, documented by Bartov and Konchitchki (2017), is reproducible on an out-of-sample window when (i) the equity return source is replaced with the Center for Research in Security Prices (CRSP) daily file including delisting returns and (ii) the late-filing body narrative is scored by a zero-shot large language model. For 31,693 Form NT filings filed on the SEC EDGAR Form Index between 2014 and 2024, the short-window five-day cumulative abnormal return is recovered at $-2.11\%$ for Form NT 10-K ($n = 824$, $|t|=4.77$) and $-2.88\%$ for Form NT 10-Q ($n = 1{,}114$, $|t|=5.55$), within $0.15$ and $0.05$ percentage points of the published baseline. A zero-shot three-class taxonomy applied to 3,232 body narratives identifies an "accounting-issue" class for which the probability of a subsequent same-filer restatement-class disclosure within ninety trading days is $32.6\%$, versus $23.5\%$ for the residual class (two-proportion $z = 5.61$). The same difference is $+8.4$ percentage points ($z = 2.83$) on a fully held-out 2025--2026 sample. A pre-specified equal-weighted long--short basket built on this label, anchored at the first trading day strictly after the filing's accepted timestamp to respect intraday tradability, delivers a net Sharpe ratio of $0.59$ at the ninety-day holding horizon on the in-sample window after a fifteen-basis-point round-trip cost. The held-out window has insufficient monthly periods to re-test the basket directly; the underlying classification signal retains its statistical significance out of sample. The contribution is methodological rather than channel-discovery: we (i) document that a free-tier equity feed misstates the post-filing drift sign because of delisted-firm self-selection, (ii) extend the existing channel with a body-narrative-based forward signal that survives multiple-comparison correction across twenty-four cell specifications, and (iii) supply a one-command end-to-end replication.
jel: G14, M41, C58
...
The U.S. Securities and Exchange Commission's Rule 12b-25 requires that a registrant unable to file its periodic report on Form 10-K or Form 10-Q on time submit, no later than the next business day, a Form NT 10-K or Form NT 10-Q together with a written explanation of the delay in Part III of Form 12b-25. The disclosure carries a sign: @bartovkonchitchki2017 report a cumulative abnormal return of $-1.96\%$ around the Form NT 10-K filing and $-2.93\%$ around the Form NT 10-Q filing over the five-day $(-2,+2)$ window on a hand-collected 2003--2011 sample, and document a continuing downward drift over the subsequent post-filing window. Their central economic interpretation is that the failure to file on time signals an interruption in financial reporting quality that itself predicts subsequent restatements and write-downs.

This paper asks two methodological questions. *First*, does the short-window magnitude survive sample reconstruction on an eleven-year window outside the Bartov--Konchitchki calendar, once the typical free-tier equity feed is replaced with a return source that includes delisting returns? *Second*, can a zero-shot text classification of the body narrative, performed without exposure to any return-based training label, provide a forward signal for subsequent restatement disclosure that exceeds a Bonferroni threshold over twenty-four cell-level specifications?

The answers are interesting on both sides. The first question is non-trivial because the natural open-source replication path --- a free-tier ticker-keyed equity feed --- omits delisted firms by construction. On that feed the short-window magnitude is under-recovered (Form NT 10-Q comes in at $-0.72\%$ rather than the published $-2.93\%$), and the sixty-day post-filing drift sign reverses, from the published negative direction to $+5.50\%$ on the Form NT 10-Q sample. Replacing the feed with the Center for Research in Security Prices (CRSP) daily file including delisting returns recovers both the magnitude (within $0.5$ percentage points) and the direction. The free-tier feed's bias is decomposed and reported in its own right. The second question is non-trivial because the existing literature on text-mining of SEC filings for cross-sectional equity signals [@johnstonpetacchi2017; @ryans2021; @lopezliratang2023] does not address Form NT body narratives. The natural body-narrative signal, moreover, is cleanly distinguishable from the short-window event signal by virtue of its forward window: the short-window CAR is realized at the filing date, while the body-narrative signal is intended to predict subsequent restatement-class filings.

We organize the paper around four contributions.

First, we document a quantified delisting-return artifact in free-tier replication. On a free-tier ticker-keyed equity feed the published short-window CAR magnitudes are under-recovered, and the sixty-day post-filing drift reverses sign on the Form NT 10-Q sample. The artifact's implied magnitude on the delisted-firm subset is consistent with the literature on delisting-return economics [@shumway1997]. Replacing the free-tier feed with the CRSP daily file including delisting returns recovers the short-window CAR within $0.15$ percentage points for Form NT 10-K and $0.05$ percentage points for Form NT 10-Q.

Second, we extend the established Form NT short-window channel with a body-narrative forward signal. A zero-shot three-class taxonomy applied to the body narrative identifies an "accounting-issue" class for which the probability of a subsequent same-filer 8-K Item 4.02 (non-reliance), 10-K/A, or 10-Q/A within ninety trading days is materially higher than for a residual class. The signal survives a family-wise correction over twenty-four cell-level specifications spanning three test angles, two cohort cuts, and four post-filing windows; it is also preserved out of sample on a fully held-out 2025--2026 cohort.

Third, we construct a pre-specified long--short basket that respects intraday tradability. The basket is anchored at the first trading day strictly after the filing's accepted timestamp, because $63\%$ of in-sample Form NT filings are accepted by the SEC after the $16{:}00$ Eastern Time market close on the calendar filing date and are therefore not investable at that close. The net Sharpe ratio at the ninety-day holding horizon, after a fifteen-basis-point round-trip transaction cost, is $0.59$ on the in-sample window, with a break-even cost of $320$ basis points round-trip. The held-out window's monthly-period sample is too thin to re-test the basket directly; we report this limitation honestly rather than over-claim deployment-ready performance.

Fourth, we release a one-command end-to-end replication, including the body-narrative extraction prompt, the CRSP join, the body-text classification step, and the post-filing event-window construction.

The rest of the paper proceeds as follows. Section 2 places the study in the existing literature. Section 3 describes the data. Section 4 develops the methodology. Section 5 reports the cell-level results, the side-by-side comparison with @bartovkonchitchki2017, and the robustness battery, with the basket's cumulative profit-and-loss reproduced in Figure 1. Section 6 discusses limitations honestly. Section 7 concludes.
...
This study is therefore a methodological extension of @bartovkonchitchki2017 along five dimensions and not a discovery claim on the underlying channel.
...
A two-vendor cross-check is performed on a stratified random sample of $n = 50$ narratives drawn with seed 42 from the in-sample pool. The same prompt is applied with a different vendor's model. The resulting Cohen's $\kappa$ on the $n = 49$ shared classifications is $0.7066$ with observed agreement of $85.7\%$. The taxonomy is retained at the three-class specification.
...
\caption{Cumulative excess return of the long--short basket on the body-narrative classification, in-sample 2014--2024. Each monthly observation is the realized ninety-trading-day excess return on the corresponding month-end basket, net of fifteen basis points round-trip per entry. The plotted series is the running sum across $58$ usable monthly periods. Annualization uses $252/90 \approx 2.8$ periods per year. The compound-growth interpretation of cumulative excess return is suppressed to avoid the overlap distortion that would arise from monthly-frequency compounding of a strategy whose holding period is ninety days.}
\end{figure}
...
\caption{Cell-level $|t|$ or $|z|$ statistic for the expanded twenty-four-cell family. Bars above the dashed line clear the family-wise critical value. Hatched bars fail magnitude or direction. The recurring-filer test (red) and the event-window CAR test (blue) clear the critical value at every specification; the body-narrative forward signal (green) clears on rate-difference cells but not on within-horizon CAR-difference cells, consistent with the market having priced the body content at the filing date.}
\end{figure}
...
We catalog limitations directly rather than relegating them to a footnote.
...
The body-narrative classifier is operated zero-shot. A pre-2024-vintage model would supply an additional defense against the concern that the classifier's pre-training corpus may encode forward outcome information. The two-vendor cross-check Cohen's $\kappa$ of $0.71$ clears the pre-specified $0.70$ floor at the lower bound, not at the comfortable middle; a re-extraction with an earlier generation of models is a straightforward monitoring step at negligible cost.
...
The recurring-filer cross-section is constructed on a cohort heavily concentrated in small market-capitalization filers. Realistic transaction costs and locate-and-borrow availability for short legs may exceed the fifteen-basis-point baseline; the break-even cost of $320$ basis points round-trip leaves room but is not unlimited.

\clearpage

# Conclusion

We have tested whether the negative cumulative abnormal return around SEC Form NT 10-K and Form NT 10-Q late-filing notifications, documented by @bartovkonchitchki2017 on 2003--2011 data, is reproducible on a recent eleven-year window using a return source that includes delisting returns, and whether a zero-shot text classification of the late-filing body narrative produces a forward signal for subsequent restatement disclosure that survives multiple-comparison correction.

On the short-window event, the published five-day CAR magnitudes are recovered within $0.15$ and $0.05$ percentage points (Forms NT 10-K and NT 10-Q respectively) on a CRSP-with-delisting-return sample reconstruction. The same regression on a free-tier ticker-keyed feed under-recovers the magnitude and reverses the sixty-day post-filing drift sign, an artifact whose implied delisted-firm mean forward return is consistent with the empirical literature on delisting returns. The artifact is decomposed quantitatively and reported as a stand-alone methodological contribution.

On the forward signal, the body-narrative-conditioned probability of a same-filer restatement-class disclosure within ninety trading days is $32.6\%$ for the accounting-issue class and $23.5\%$ for the other class ($z = 5.61$). The same difference is $+8.35$ percentage points ($z = 2.83$) on a fully held-out 2025--2026 cohort. Eighteen of twenty-four cells pass a family-wise correction over the expanded specification.

On the tradeable construction, a long--short basket built on the body-narrative classification delivers a ninety-day net Sharpe ratio of $0.59$ under an acceptance-tradable anchor (entry on the first trading day strictly after the SEC accepted timestamp for after-hours filings) after a fifteen-basis-point round-trip cost. Cumulative growth is plotted in Figure 1. The break-even round-trip cost at the deployment pass floor is $320$ basis points. The held-out window has only five usable monthly periods at the ninety-day horizon and we do not draw a deployment-ready conclusion from it.

The contribution is methodological rather than channel-discovery: we reproduce the @bartovkonchitchki2017 channel under a different return source and a different filing universe, extend it with a zero-shot body-narrative forward signal that survives a twenty-four-cell family-wise correction and a held-out window, and supply a one-command replication. The full pipeline, including the classification prompt and the replication scripts, is publicly released.

...
Residual $\alpha$ trail (rescaled to tangent vol) & 58 & $\phantom{0}7.24\%$ & $\phantom{0}4.67\%$ & --- & $1.55$ \\
...
\caption{Growth of one dollar of capital at a common $4.67\%$ annualized risk budget, in-sample 2014--2024. The three series are: (i) the body-narrative long--short basket rescaled to the tangent's natural unleveraged volatility (blue solid); (ii) the cumulative residual alpha trail (green dot-dash), which is the part of the strategy's per-capital return that survives the projection onto the six Fama-French + momentum factors, accumulated as a cash-balance series and rescaled to the same tangent volatility; and (iii) the FF5+UMD maximum-Sharpe tangent portfolio at its natural unleveraged scale (red dashed, sum of absolute weights equal to one). All three series share the same risk budget, so terminal capital values can be read on a same-volatility basis. The inset reports each series's terminal capital and annualized Sharpe ratio, together with the per-capital factor-regression alpha and $R^2$ reported in Table 6. The tangent baseline is in-sample optimal and therefore upward-biased by mean-variance optimism; its Sharpe of $1.96$ should be read as an upper bound on what a tangent-style strategy on the same factors could deliver out of sample.}
\end{figure}

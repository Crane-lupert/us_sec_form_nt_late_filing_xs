...
\caption{Cumulative additive return of the long--short basket on the body-narrative classification, in-sample 2014--2024, against a buy-and-hold SPDR S\&P 500 ETF (SPY) benchmark on the same months. Each monthly strategy observation is the realized ninety-trading-day excess return on the corresponding month-end basket, net of fifteen basis points round-trip per entry; each monthly benchmark observation is the SPY buy-and-hold return over the same calendar month. Both series are plotted as the running sum (additive convention) so they share a common scale. The long--short basket is already market-neutral by construction, so the SPY line is provided as a directional long-only equity reference rather than as a strict head-to-head excess-return benchmark. Annualization uses $252/90 \approx 2.8$ periods per year for the strategy. The compound-growth interpretation of cumulative excess return is suppressed to avoid the overlap distortion that would arise from monthly-frequency compounding of a strategy whose holding period is ninety days.}
\end{figure}

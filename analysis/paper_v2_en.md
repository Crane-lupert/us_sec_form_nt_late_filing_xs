...
Finding 3. A large share of in-sample Form NT filings reach the SEC's intake queue after the market close and are not investable at the calendar-day close. The measured share is $63.5\%$ on the 2014--2024 window. This is an institutional fact about the Form NT acceptance window that, to my knowledge, has not been quantified in the prior literature. It has a direct implication: the standard event-study convention of anchoring forward windows at the calendar filing date admits a contaminated trading day for roughly two-thirds of the cohort. Anchoring entry at the first trading day strictly after the SEC accepted timestamp lifts the long--short basket's in-sample per-position net Sharpe at the ninety-day holding horizon from $0.46$ (calendar anchor) to $0.59$ (acceptance-tradable anchor), after a fifteen-basis-point round-trip cost. Per dollar of deployed capital --- the strategy holds approximately four overlapping ninety-day positions --- the corresponding per-capital Sharpe is approximately $0.29$ under independence; the per-position number is the position-level metric and the per-capital number is the deployment-level metric. A Fama-French five-factor plus momentum regression on the monthly long--short return yields a per-position annualized residual alpha of $+31.5$ percentage points ($t = 2.14$, Newey-West HAC) with $R^2 = 9.4\%$ (per capital approximately $+15$ percentage points), indicating that the strategy's profitability is not a reproduction of common factor exposure. The cumulative growth profile of the basket against the Fama-French five-factor plus momentum factor-spanned portfolio --- the part of the strategy's return that common factor exposure can replicate --- is reproduced in Figure 1; the vertical gap between the strategy and the factor-spanned series is the realized residual alpha trail.
...
A complementary tradability concern is that $63.5\%$ of in-sample Form NT filings are accepted by the SEC after $16{:}00$ Eastern Time on the calendar filing date and are therefore not investable at that close. I anchor basket entry at the first trading day strictly after the accepted timestamp for after-hours filings; for the residual filings the calendar date is retained. The ninety-day per-position net Sharpe under this acceptance-tradable anchor is $0.59$ ($n = 58$ usable monthly periods, annualized mean $25.81\%$, annualized volatility $43.69\%$). Because each monthly entry holds for ninety trading days, a deployed dollar of capital carries approximately $90 / 21 \approx 4.3$ simultaneous overlapping positions; the deployment-relevant per-capital Sharpe is approximately $0.59 / \sqrt{4.3} \approx 0.29$ under an independence approximation across overlapping positions, and is bounded above by the per-position value and below by the independence approximation depending on the realized cross-position covariance. Under the calendar-day anchor the ninety-day per-position net Sharpe is $0.46$ ($n = 53$); the magnitudes are reported side-by-side in Table 4. The per-position convention is the natural unit for a position-level Sharpe report, and is the convention used in the remainder of this section unless otherwise noted; the per-capital companion is the deployment-relevant figure.

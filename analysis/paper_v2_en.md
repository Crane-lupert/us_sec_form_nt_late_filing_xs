...
\caption{Cumulative additive return of the long--short basket on the body-narrative classification against a same-horizon SPDR S\&P 500 ETF (SPY) benchmark, in-sample 2014--2024. The strategy series records, at each month-end, the realized ninety-trading-day excess return on the body-narrative basket net of fifteen basis points round-trip per entry. The benchmark series records, at the same month-ends, the SPY buy-and-hold return over the next ninety trading days. The two series therefore share an identical cadence and holding horizon and are plotted as the running sum in additive convention. SPY is the natural market benchmark because the abnormal-return computation throughout Sections 4 and 5 already uses SPY as the market subtractor in the firm-minus-market construction. The basket is already market-neutral by construction, so the SPY line is provided as a same-horizon long-only reference rather than as a strict head-to-head excess-return benchmark. The compound-growth interpretation is suppressed to avoid the overlap distortion that would arise from monthly-frequency compounding of a strategy whose holding period is ninety days.}
\end{figure}
...
On the tradeable construction, a long--short basket built on the body-narrative classification delivers a ninety-day net Sharpe ratio of $0.59$ under an acceptance-tradable anchor (entry on the first trading day strictly after the SEC accepted timestamp for after-hours filings) after a fifteen-basis-point round-trip cost. Cumulative growth, with a buy-and-hold SPDR S\&P 500 ETF (SPY) benchmark, is plotted in Figure 1. The break-even round-trip cost at the deployment pass floor is $320$ basis points. The held-out window has only five usable monthly periods at the ninety-day horizon and I do not draw a deployment-ready conclusion from it.

The contribution is methodological rather than channel-discovery: I reproduce the @bartovkonchitchki2017 channel under a different return source and a different filing universe, extend it with a zero-shot body-narrative forward signal that survives a twenty-four-cell family-wise correction and a held-out window, and supply a one-command replication. The full pipeline, including the classification prompt and the replication scripts, is publicly released.

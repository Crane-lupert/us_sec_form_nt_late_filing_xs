...
This paper introduces a zero-shot language-model classification of the body narrative of SEC Form NT 10-K and Form NT 10-Q filings and shows that the resulting "accounting-issue" class is a strong forward signal for subsequent same-filer restatement-class disclosure. On in-sample data the body-narrative-conditioned probability of a same-filer 8-K Item 4.02, 10-K/A, or 10-Q/A within ninety trading days is $32.6\%$ for the accounting-issue class against $23.5\%$ for the residual class ($z = 5.61$). The signal is preserved out of sample, with a $+8.35$ percentage point difference ($z = 2.83$) on a fully held-out 2025--2026 cohort. Eighteen of twenty-four cell specifications pass a family-wise correction across three test angles, two cohort cuts, and four forward windows. To my knowledge, the body-narrative forward signal is new to the literature, as are the Form NT 10-Q cohort dominance ($z = 6.77$ on the cohort-split rate-difference cell against a null $z = 0.50$ on the Form NT 10-K cohort) and the institutional fact that $63.5\%$ of in-sample filings reach the SEC's intake queue after the $16{:}00$ Eastern Time market close.

A pre-specified long--short basket on the classification, anchored at the first trading day strictly after the filing's accepted timestamp to respect intraday tradability, delivers a ninety-day net Sharpe ratio of $0.59$ in sample after a fifteen-basis-point round-trip cost, against a break-even cost of $320$ basis points. The strategy's cumulative additive return reaches approximately $540$ percentage points across the eleven-year sample, against approximately $287$ percentage points for a same-cadence same-horizon SPDR S\&P 500 ETF (SPY) benchmark (Figure 1). The held-out window provides only five usable monthly periods at the ninety-day horizon and I therefore do not draw a deployment-ready conclusion from it; the underlying rate-difference signal retains its statistical significance on the same window, which suggests that the small-sample failure is in the basket's monthly aggregation step rather than in the classification.

The published @bartovkonchitchki2017 short-window cumulative abnormal return magnitudes are recovered within $0.15$ and $0.05$ percentage points for Form NT 10-K and Form NT 10-Q respectively on a CRSP-with-delisting-return sample reconstruction. The same regression on a free-tier ticker-keyed equity feed under-recovers the magnitude and reverses the sixty-day post-filing drift sign --- a free-tier delisting-return artifact that I quantify as a stand-alone methodological caveat for recent open-source replications.

The full pipeline, including the classification prompt, the same-horizon SPY benchmark series, the CRSP return join, and the held-out 2025--2026 cohort scripts, is publicly released.

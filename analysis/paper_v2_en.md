...
FF5+UMD max-Sharpe (vol-matched) reference & --- & $40.17\%$ & $23.01\%$ & --- & $1.75$ \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Growth of one dollar of capital, in-sample 2014--2024, under an explicit per-capital back-test. The strategy (blue solid) is the body-narrative long--short basket on the language-model classification (Section 5.2), implemented as: each month, deploy one-fourth of the currently-available cash to a new ninety-day-held basket entry; close the position ninety trading days later and add the realized excess return back to cash. The basket is net of fifteen basis points round-trip per entry. The Fama-French five-factor plus momentum maximum-Sharpe tangent portfolio (red dashed) is the in-sample mean-variance optimal combination of the six Ken French factor return series (Mkt-RF, SMB, HML, RMW, CMA, MOM), re-scaled so that the realized monthly portfolio volatility annualizes to the strategy's $22.82\%$; this is the fair quantitative factor baseline for a long--short strategy with this risk budget. It is in-sample optimal and therefore upward-biased by the standard mean-variance optimism, so its realized Sharpe should be read as an upper bound on what a tangent-style strategy on the same factors could deliver out of sample. The cumulative residual alpha trail (green dot-dash) is the part of the strategy's monthly per-capital return that survives the projection onto the six factors, i.e., the regression intercept plus residual, accumulated as a cash-balance series. The vertical gap between the strategy and the residual alpha trail is the cumulative contribution of the strategy's factor exposure, which is net negative over the sample, so the residual alpha trail ends above the strategy at $\$4.00$ even though the strategy's raw terminal is $\$3.03$. All three series are on the same one-dollar-of-capital basis. Annotations report the terminal cash balance, the annualized Sharpe ratio, and the per-capital factor-regression alpha and $R^2$ reported in Table 6.}
\end{figure}
